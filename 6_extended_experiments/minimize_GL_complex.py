...
def run_model(inverse_model, optimizers, E_target, n_epoch, out_tag, schedulers=None, print_freq=10):
    ...
    for epoch in range(n_epoch):
        ...
        for optimizer in optimizers:
            optimizer.zero_grad()
        loss.backward()
        # Addition for aggresstive 3T
        n_micro_ligand = len(inverse_model.macro_mode_idx[-1])
        n_micro_all = inverse_model.rotation_list.shape[0]
        for k in range(n_micro_all-n_micro_ligand, n_micro_all):
            inverse_model.movable_pos_list[k].grad *= 3
            inverse_model.rotation_list.grad[-n_micro_ligand:,:] *= 3
            inverse_model.translation_list.grad[-n_micro_ligand:,:] *= 3
            inverse_model.macro_mode_rotation_list.grad[-1,:] *= 3
            inverse_model.macro_mode_translation_list.grad[-1,:] *= 3
        # End addition
...
def main(pid, convert_folder, epochs, init_shift, kick_prop, print_freq):
    ...
    # Addition for aggressive 3T
    print_log( 'Special bad case treatment: give big initial biased translation and big initial random rotation' )
    ...
    if os.path.isfile(out_xyz):
        ...
        inverse_model.attach_init_inputs(xyz, movable_index_list, special_rotation=special_rotation, macro_mode=macro_mode)
        # Addition for aggressive 3T
        for atom_pos in inverse_model.movable_pos_list:
            atom_pos.requires_grad = False
            atom_pos[:,0] += init_shift[0]
            atom_pos[:,1] += init_shift[1]
            atom_pos[:,2] += init_shift[2]
            atom_pos.requires_grad = True
        torch.manual_seed(seed)
        max_rot = 3.14
        rotation_shape = [1, inverse_model.macro_mode_rotation_list.shape[1]]
        inverse_model.macro_mode_rotation_list.requires_grad = False
        inverse_model.macro_mode_rotation_list[-1,:] = (torch.rand(rotation_shape) - 0.5) * 2 * max_rot
        inverse_model.macro_mode_rotation_list.requires_grad = True
        # End addition
        ...
        optimizer = optim.Adam( optim_params , 1e-1, # Aggresive 3T, modify from original value 1e-3,
                               weight_decay=0)
    ...
    # Return if epochs only have 2 phases
    if len(epochs) == 2:
        ## Notify job creator of complex minimization completion
        print_log('Finished minimizing complex | '+complex_tag+' | '+str(kick_prop))
        return

    ## We need to do one more round of protein and ligand minimization cycle without additional kick
    print_log( 'Minimizing both ligand and nearby protein pocket structure without additional energetic kick' )
    center_pos, dist_cutoff, strength, seed = kick_prop
    center_pos = np.array(center_pos)
    movable_protein_idx = segmentize_protein_atoms_from_center(data_struct, ligand_idx, center_pos, dist_cutoff)
    movable_index_list = movable_protein_idx.copy()
    ...
    E_target = 0
    n_epoch = epochs[2]
    out_tag = os.path.join(out_folder, '_'.join(['step3'] + [str(i) for i in epochs[0:3]] + [str(strength),str(seed)]) )
    out_xyz = out_tag+'.xyz'
    if os.path.isfile(out_xyz):
        # Grab last snapshot and update the atom positions in the model
        # Because of string-float conversion, it'll be slightly different
    ...
    ## Combine output energy files for convenience purposes
    all_out_E = os.path.join(out_folder, '_'.join(['stepAll'] + [str(i) for i in epochs[0:3] + [str(strength),str(seed)] + ['outE.txt']]) )
    append_out_E_files(all_out_E, out_E_files)
