    ...
    def read(self):
        ...
        with open(self.gro_file) as gro:
            ...
            for _ in range(n_atoms):
                ...
                (thisresnum, thisresname, thisatomname) = [words[i] for i in range(3)]
                resname.append(thisresname)
                #resid.append(int(thisresnum))
                if not (thisresnum in hack_dict):
                    hack_dict[thisresnum] = int(len(hack_dict.keys()) + 1)
                resid.append( hack_dict[thisresnum] )
